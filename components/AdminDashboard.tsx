...
import MasterFloorPlanV2 from "./MasterFloorPlanV2";
...
export default function AdminDashboard() {
  ...
  const { user } = useUser();
  const dashboardData = useQuery(api.portal.getDashboardData, {
    clerkId: user?.id,
  });
  ...
  const updateBuildingCalibration = useMutation(
    api.portal.updateBuildingCalibration,
  );
  const updateBuildingGridPaths = useMutation(
    api.portal.updateBuildingGridPaths,
  );
  const updateBuildingDoorPins = useMutation(
    api.portal.updateBuildingDoorPins,
  );
  ...
  const recentIncidents =
    useQuery(api.portal.getRecentIncidents, { clerkId: user?.id }) || [];
  const allIncidentsHistory = useQuery(api.portal.getAllIncidentsHistory, {
    clerkId: user?.id,
  });
  const notifyFireVisionNewAdmin = useAction(
    api.emails.notifyFireVisionNewAdmin,
  );
  ...
  const hasPermissions =
    dashboardData?.locationGranted === true &&
    dashboardData?.notificationsGranted === true;
  const [setupHasLocation, setSetupHasLocation] = React.useState(false);
  const [setupHasNotifications, setSetupHasNotifications] =
  ...
  const [mapEditorMode, setMapEditorMode] = React.useState<"v1" | "v2">("v1");
  ...
  const [gridPaintMode, setGridPaintMode] = React.useState<
    "safe_route" | "exit" | "erase" | "pan" | "entrance" | null
  >(null);
  const lastPaintedCellRef = React.useRef<{ row: number; col: number } | null>(
    null,
  );
  const [gridSizeMeters, setGridSizeMeters] = React.useState(1.2);
  const scrollViewRef = React.useRef<ScrollView>(null);
  const step2ScrollViewRef = React.useRef<ScrollView>(null);
  const [userZoom, setUserZoom] = React.useState(1);
  const [step1Zoom, setStep1Zoom] = React.useState(1);
  const [step1PanMode, setStep1PanMode] = React.useState<boolean | null>(null);
  const [step1MapScrollOffsetY, setStep1MapScrollOffsetY] = React.useState(0);
  const [step2MapScrollOffsetY, setStep2MapScrollOffsetY] = React.useState(0);
  ...
  const [calibPoints, setCalibPoints] = React.useState<
    { x: number; y: number }[]
  >([]);
  const [gridPaths, setGridPaths] = React.useState<
    {
      row: number;
      col: number;
      lat: number;
  ...
  const [doorPins, setDoorPins] = React.useState<
    {
      x: number;
      y: number;
      type: string;
  ...
  >([]);
  const [step2PinType, setStep2PinType] = React.useState<"entrance" | "exit" | "pan" | null>(null);
  const [activeDoorPinIdx, setActiveDoorPinIdx] = React.useState<number>(-1);
  const [imgLayout, setImgLayout] = React.useState<{ w: number; h: number }>({
    w: 1,
    h: 1,
  });
  ...
  const [editingPins, setEditingPins] = React.useState<
  ...
  const [confirmDialog, setConfirmDialog] = React.useState({
    ...
    message: "",
    onConfirm: () => {},
    intent: "success" as "success" | "warning" | "danger",
  });

  const [manageSiteName, setManageSiteName] = React.useState<string | null>(
    null,
  );
  ...
  const [bPins, setBPins] = React.useState<
    { lat: number; lon: number; label?: string }[]
  >([]);
  const [bImageUri, setBImageUri] = React.useState<string | null>(null);
  const [bImageMimeType, setBImageMimeType] = React.useState<string | null>(
    null,
  );
  ...
  React.useEffect(() => {
    if (selectedBuilding && dashboardData?.buildings) {
      const updated = dashboardData.buildings.find(
        (b: any) => b._id === selectedBuilding._id,
      );
      if (updated && updated.masterPlanUrl !== selectedBuilding.masterPlanUrl) {
        setSelectedBuilding((prev: any) => ({
          ...prev,
          masterPlanUrl: updated.masterPlanUrl,
  ...
  const locatingIncidentId = React.useMemo(() => {
    ...
    const inc = activeIncidents.find(
      (i: any) => i.buildingId === selectedBuilding._id,
    );
    return inc?.incidentId;
  }, [isLocatingUser, selectedBuilding, activeIncidents]);

  const rawEvacData = useQuery(
    api.portal.getEvacuationData,
    locatingIncidentId
      ? { clerkId: user?.id || "", incidentId: locatingIncidentId }
      : "skip",
  );
  ...
  const getGridDimensions = () => {
    if (!selectedBuilding?.polygon || selectedBuilding.polygon.length < 4)
      return {
        rows: 1,
        cols: 1,
        minLat: 0,
    ...
    const cellLatSpan =
      heightMeters > 0
        ? ((maxLat - minLat) / heightMeters) * gridSizeMeters
        : 0;
    const cellLonSpan =
      widthMeters > 0 ? ((maxLon - minLon) / widthMeters) * gridSizeMeters : 0;
    ...
    if (touches && touches.length >= 2) {
      ...
      if (isPanMode === false && onPaint) {
        ...
        if (Platform.OS === "web") {
          ...
          lx =
            touches?.length > 0 && touches[0].locationX !== undefined
              ? touches[0].locationX
              : e.nativeEvent.locationX;
          ly =
            touches?.length > 0 && touches[0].locationY !== undefined
              ? touches[0].locationY
  ...
  const handleTouchMove = (
    ...
    if (touches && touches.length >= 2) {
      ...
      const scaleFactor =
        dist / Math.max(1, gestureState.current.initialDistance);
      ...
      } else if (isPanMode === false) {
        if (onPaint) {
          const mapRef =
            onPaint === handleStep1Paint ? step1MapRef : step2MapRef;
          let rawX, rawY;
          if (Platform.OS === "web") {
            if (mapRef.current) {
              const rect = (mapRef.current as any).getBoundingClientRect();
  ...
  const getMapTransform = () => {
    ...
    if (
      selectedBuilding?.imageCalibrationPoints?.length >= 4 &&
      imgLayout.w > 1
    ) {
      ...
      const minCX = Math.min(
        ...calib.map((c: any) =>
          isLegacyPixels ? c.x / Math.max(1, imgLayout.w) : c.x,
        ),
      );
      ...
      const minCY = Math.min(
        ...calib.map((c: any) =>
          isLegacyPixels ? c.y / Math.max(1, imgLayout.h) : c.y,
        ),
      );
  ...
  const handleStep1Paint = (rawX: number, rawY: number) => {
    ...
    const unzoomedX =
      imgLayout.w / 2 +
      (screenX - imgLayout.w / 2 - step1PanOffset.x) / step1Zoom;
    const unzoomedY =
      imgLayout.h / 2 +
      (screenY - imgLayout.h / 2 - step1PanOffset.y) / step1Zoom;
  ...
  const handleStep2DoorPinPaint = (rawX: number, rawY: number) => {
    const bounds = getRenderedImageBounds();
    const screenX = rawX;
    const screenY = rawY;
    const unzoomedX =
    ...
    if (!step2PinType && activeDoorPinIdx === -1) return;
    ...
    if (activeDoorPinIdx === -1) {
       ...
       // move the active pin
       const newPins = [...doorPins];
       newPins[activeDoorPinIdx] = { ...newPins[activeDoorPinIdx], x, y };
       setDoorPins(newPins);
    }
  };

  const handleNudgeDoorPin = (idx: number, dx: number, dy: number) => {
    if (idx < 0 || idx >= doorPins.length) return;
    const newPins = [...doorPins];
    newPins[idx] = {
      ...newPins[idx],
      x: newPins[idx].x + dx * 0.001,
      y: newPins[idx].y + dy * 0.001,
    };
    setDoorPins(newPins);
  };
  ...
  const handleGridInteraction = (screenX: number, screenY: number) => {
    ...
    const unzoomedX =
      imgLayout.w / 2 +
      (screenX - imgLayout.w / 2) / step1Zoom -
      step1PanOffset.x;
    const unzoomedY =
      imgLayout.h / 2 +
      (screenY - imgLayout.h / 2) / step1Zoom -
      step1PanOffset.y;
    ...
    try {
      ...
      for (const inc of allIncidentsHistory) {
        ...
        const durationSecs = Math.floor(
          ((inc.resolvedAt || Date.now()) - inc.triggeredAt) / 1000,
        );
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, {
          mimeType: "text/csv",
          dialogTitle: "Export Evacuation Logs",
        });
      } else {
  ...
  const confirmAction = (
    ...
    message: string,
    onConfirm: () => void,
    intent: "success" | "warning" | "danger" = "success",
  ) => {
  ...
  React.useEffect(() => {
    if (selectedBuilding && dashboardData?.buildings) {
      if (selectedBuilding) {
        ...
        if (
          updated &&
          (updated.imageCalibrationPoints?.length !==
            selectedBuilding.imageCalibrationPoints?.length ||
  ...
  const mapImageToGPS = (rawX: number, rawY: number) => {
    ...
    const calib =
      calibPoints.length >= 4
        ? calibPoints
        : selectedBuilding.imageCalibrationPoints;
  ...
  const mapGPSToImage = (lat: number, lon: number) => {
    ...
    const currentCalib =
      calibPoints.filter(Boolean).length >= reqPins
        ? calibPoints.filter(Boolean)
        : selectedBuilding.imageCalibrationPoints;
    if (currentCalib && currentCalib.length >= reqPins) {
      const isLegacyPixels = currentCalib[0].x > 2;
      const minCX = Math.min(
        ...currentCalib.map((c: any) =>
          isLegacyPixels ? c.x / Math.max(1, imgLayout.w) : c.x,
        ),
      );
      const maxCX = Math.max(
  ...
  const handleNudgeCalib = (idx: number, dx: number, dy: number) => {
    ...
    const rawX =
      (isLegacyPixels ? p.x : bounds.offsetX + p.x * bounds.renderW) + dx * 2;
    const rawY =
      (isLegacyPixels ? p.y : bounds.offsetY + p.y * bounds.renderH) + dy * 2;
  ...
  const performAutoZoom = () => {
    if (imgLayout.w > 1 && calibPoints.filter(Boolean).length >= 4) {
      ...
      const xs = validPoints.map((c: any) =>
        isLegacyPixels ? c.x / Math.max(1, imgLayout.w) : c.x,
      );
      const ys = validPoints.map((c: any) =>
        isLegacyPixels ? c.y / Math.max(1, imgLayout.h) : c.y,
      );
      ...
      if (xs.length > 0 && ys.length > 0) {
        ...
        if (boxW_px > 0 && boxH_px > 0) {
          ...
          const boxCenterX =
            bounds.offsetX + ((minCX + maxCX) / 2) * bounds.renderW;
          const boxCenterY =
            bounds.offsetY + ((minCY + maxCY) / 2) * bounds.renderH;
  ...
  const handleMapPress = (e: any) => {
    if (e.nativeEvent.coordinate) {
      ...
      const label =
        bPins.length < 4 ? defaultLabels[bPins.length] : `P${bPins.length + 1}`;
      setBPins([
        ...bPins,
        {
          lat: parseFloat(e.nativeEvent.coordinate.latitude.toFixed(6)),
          lon: parseFloat(e.nativeEvent.coordinate.longitude.toFixed(6)),
          label,
        },
      ]);
    }
  ...
  const handleSaveBuilding = async () => {
    if (bPins.length > 0 && bPins.length < 3) {
      showToast(
        "Please drop at least 3 pins to create a polygon footprint, or leave it empty to configure later.",
      );
      return;
    }
      ...
      if (bImageUri) {
        try {
          ...
          const uploadResult = await fetch(postUrl, {
            method: "POST",
            headers: {
              "Content-Type": bImageMimeType || blob.type || "image/jpeg",
            },
            body: blob,
          });
          ...
          showToast(
            "Failed to upload image. Saving building without image.",
            "error",
          );
        }
      }
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            Alert.alert("Select Source", "Where is your file located?", [
              {
                text: "Photo Gallery",
                onPress: async () => {
                  const libPerm =
  ...
  const needsSetup =
    !dashboardData.name || !dashboardData.phone || !dashboardData.businessName;
            ...
            <View>
              ...
              <View className="bg-neutral-900 p-4 rounded-xl mb-4 border border-neutral-700">
                <Text className="text-neutral-400 text-sm">
                  Note: I agree to pay a subscription fee once my account is
                  approved. (Currently in Test Mode, subscription details will
                  be announced on firevision.uk in the near future).
                </Text>
              </View>
              ...
              <View className="mb-4">
                  ...
                  onPress={async () => {
                    if (setupHasLocation) {
                      ...
                      try {
                        ...
                        if (status !== "granted") {
                          alert(
                            "Location permission denied by OS. Please enable it in your device settings.",
                          );
                        }
                      } catch (e) {
                        console.warn(e);
                ...
                >
                  <View
                    className={`w-6 h-6 rounded border items-center justify-center mr-3 ${setupHasLocation ? "bg-blue-600 border-blue-500" : "bg-neutral-900 border-neutral-700"}`}
                  >
                    {setupHasLocation && (
                    ...
                    if (setupHasNotifications) {
                      ...
                      try {
                        if (Platform.OS !== "web") {
                          ...
                          if (status !== "granted") {
                            alert(
                              "Notification permission denied by OS. Please enable it in your device settings.",
                            );
                          }
                        }
                      } catch (e) {
                ...
                >
                  <View
                    className={`w-6 h-6 rounded border items-center justify-center mr-3 ${setupHasNotifications ? "bg-blue-600 border-blue-500" : "bg-neutral-900 border-neutral-700"}`}
                  >
                    {setupHasNotifications && (
                      <Text className="text-white font-bold text-xs">✓</Text>
                    )}
                  </View>
                  <Text className="text-neutral-300 flex-1">
                ...
                >
                  ...
                  <View>
                    <Text className="text-neutral-400 text-xs uppercase mb-1 font-bold">
                      Current Coordinates
                    </Text>
                    {location ? (
                      ...
                      <Text className="text-neutral-500 italic text-xs">
              ...
              <View className="flex-row flex-wrap items-center bg-neutral-900 border border-neutral-700 p-4 rounded-2xl">
                ...
                >
                  <View
                    className={`w-6 h-6 rounded border items-center justify-center mr-3 ${setupAgreedToTandC ? "bg-blue-600 border-blue-500" : showValidation && !setupAgreedToTandC ? "bg-red-900/50 border-red-500" : "bg-neutral-900 border-neutral-700"}`}
                  >
                    {setupAgreedToTandC && (
                      <Text className="text-white font-bold text-xs">✓</Text>
                    )}
                  ...
                  <Text
                    className={`${showValidation && !setupAgreedToTandC ? "text-red-400" : "text-neutral-300"}`}
                  >
                ...
                <TouchableOpacity
                  onPress={() =>
                    Linking.openURL("https://www.firevision.uk/terms")
                  }
                >
                  <Text
                    className={`underline ${showValidation && !setupAgreedToTandC ? "text-red-400" : "text-neutral-300"}`}
                  >
              ...
              onPress={async () => {
                ...
                if (needsSetup) {
                  ...
                  if (!domain || publicDomains.includes(domain)) {
                    showToast(
                      "Public email domains are not allowed. Please register with a corporate domain.",
                      "error",
                    );
                    return;
                  }

                  const bNameRaw = setupBusinessName
                    .toLowerCase()
                    .replace(/[^a-z0-9]/g, "");
                  const domainName = domain ? domain.split(".")[0] : "";
                  const minRequiredLength = Math.max(
                    3,
                    Math.floor(bNameRaw.length / 2),
                  );
                  ...
                  if (
                    ...
                    !domainName ||
                    (!domainName.includes(
                      bNameRaw.substring(0, minRequiredLength),
                    ) &&
                      !bNameRaw.includes(
                        domainName.substring(0, minRequiredLength),
                      ))
                  ) {
                    alert(
                      "Your email domain does not appear to match your business name. Please register with a matching corporate email, or contact info@firevision.uk with your details for manual approval.",
                    );
                    return;
                  }
                }
            ...
            >
              {isSavingSetup ? (
                ...
                <Text className="text-white font-bold text-lg">
                  {needsSetup ? "Submit for Approval" : "Save Details"}
                </Text>
              )}
            </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    if (passkey)
                      showToast(
                        "Passkey created successfully! You can now use FaceID/TouchID to sign in.",
                      );
                  } catch (e: any) {
                    showToast(
                      e.errors?.[0]?.longMessage ||
                        e.message ||
                        "Error creating passkey. Your device might not support it.",
                    );
                  }
                }}
              >
    ...
    return (
      <View className="flex-1 bg-neutral-900 justify-center items-center p-6">
        ...
        <Text className="text-neutral-300 text-center text-lg mb-8">
          Registration Successful. Firevision will confirm your details and
          contact you to activate your admin account.
        </Text>
        <TouchableOpacity
          className="bg-neutral-800 border border-neutral-700 py-3 px-8 rounded-xl"
          onPress={() => signOut()}
        >
  ...
  if (!hasPermissions) {
    return (
      <View className="flex-1 bg-neutral-900 pt-6 justify-center items-center px-6">
        ...
        <Text className="text-neutral-300 text-center text-lg mb-8">
          You must approve both location and push notification permissions to
        ...
        <View className="bg-black/50 p-4 rounded-lg w-full max-w-sm">
          ...
          <Text className="text-neutral-400 font-mono text-xs">
            locationGranted: {String(dashboardData?.locationGranted)}
          </Text>
          <Text className="text-neutral-400 font-mono text-xs">
            notificationsGranted: {String(dashboardData?.notificationsGranted)}
          </Text>
          <Text className="text-neutral-400 font-mono text-xs">
            hasPermissions: {String(hasPermissions)}
          </Text>
        </View>
      </View>
    );
    ...
    <View className="flex-1 bg-neutral-900 pt-6">
      ...
      <View className="px-6 flex-row justify-between items-center my-6">
        <View className="flex-1 mr-4">
          ...
          <Text
            ...
            ellipsizeMode="tail"
          >
            {dashboardData.name || user?.primaryEmailAddress?.emailAddress}
          </Text>
        </View>
        ...
        {(() => {
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    hasPermissions
                      ? setIsRegistering(true)
                      : showToast(
                          "Permissions required to add locations",
                          "error",
                        )
                  }
                >
          ...
          const renderBuilding = (building: any) => {
            const isComplete =
              ...
              building.imageCalibrationPoints &&
              building.imageCalibrationPoints.length >= 3 &&
              building.gridPaths &&
              building.gridPaths.some((n: any) => n.isExit);
            const activeIncident = activeIncidents.find(
              (i: any) => i.buildingId === building._id,
            );
            ...
            return (
              <View
                key={building._id}
                className={`bg-neutral-800 border ${isAlarming ? (activeIncident.isDrill ? "border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.5)]" : "border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.5)]") : "border-neutral-700"} p-4 rounded-2xl mb-4`}
              >
                <View className="flex-row justify-between items-center mb-3">
                  <View className="flex-row items-center flex-1">
                    <Text
                      className="text-white font-bold text-xl mr-2 shrink"
                      numberOfLines={1}
                    >
                      {building.name}
                    </Text>
                    <TouchableOpacity
                      ...
                      onPress={async () => {
                        ...
                        const populatedPolygon = (building.polygon || []).map(
                          (p: any, i: number) => ({
                            ...p,
                            label:
                              p.label ||
                              (i < 4 ? defaultLabels[i] : `P${i + 1}`),
                          }),
                        );
                        setSelectedBuilding({
                          ...building,
                          polygon: populatedPolygon,
                        });
                  ...
                  {!isAlarming && isComplete && (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        confirmAction(
                          "Start Test Drill",
                          `Are you sure you want to manually start a TEST DRILL for ${building.name}?`,
                          () =>
                            triggerIncident({
                              clerkId: user?.id || "",
                      ...
                      <View>
                          ...
                          onPress={() =>
                            confirmAction(
                              activeIncident.isDrill
                                ? "Resolve Drill"
                                : "Resolve Evacuation",
                              `Are you sure you want to end the ${activeIncident.isDrill ? "drill" : "evacuation"} for ${building.name}?`,
                              () =>
                                resolveIncident({
                                  clerkId: user?.id || "",
                                  buildingId: building._id,
                        ...
                        >
                          <Text className="text-white font-bold text-center">
                            ✅ Resolve{" "}
                            {activeIncident.isDrill ? "Drill" : "Evacuation"}
                          </Text>
                        </TouchableOpacity>
                      ...
                      <View className="pt-2">
                        <TouchableOpacity
                          ...
                          onPress={() =>
                            confirmAction(
                              "Trigger Evacuation",
                              `Are you sure you want to trigger the REAL evacuation for ${building.name}? This will alert all guests.`,
                              () =>
                                triggerIncident({
                                  clerkId: user?.id || "",
                                  buildingId: building._id,
                        ...
                        >
                          <Text
                            className="text-white font-bold text-center text-base tracking-widest"
                            numberOfLines={1}
                            adjustsFontSizeToFit
          ...
          return (
            <View>
              {Object.keys(sites).map((siteName) => {
                ...
                const activeIncidentsForSite = siteBuildings
                  .map((b) =>
                    activeIncidents.find((i: any) => i.buildingId === b._id),
                  )
                  .filter(Boolean);
                ...
                const isRealEmergency = activeIncidentsForSite.some(
                  (i: any) => !i.isDrill,
                );
                const siteDetail = dashboardData?.sites?.find(
                  (s: any) => s.name === siteName,
                );
                const isSiteComplete = siteBuildings.every(
                  (b) =>
                    b.polygon &&
                    b.polygon.length >= 3 &&
                    b.masterPlanId &&
                    b.imageCalibrationPoints &&
                    b.imageCalibrationPoints.length >= 3 &&
                    b.gridPaths &&
                    b.gridPaths.some((n: any) => n.isExit),
                );
                ...
                return (
                  <View
                    key={siteName}
                    className={`mb-10 bg-neutral-800/40 rounded-[32px]   ${siteStatusClass}`}
                  >
                    <View className="flex-row justify-between items-center bg-neutral-800/60 p-5 border-b border-neutral-700/50">
                      <View className="flex-row items-center flex-1">
                        <Text className="text-2xl font-black text-white tracking-wide mr-3 shrink">
                          {siteName}
                        </Text>
                        <TouchableOpacity
                          className="bg-neutral-700/50 p-3 rounded-full"
                          onPress={() => {
                            ...
                            setSiteAdminName(
                              siteDetail?.adminContactName || "",
                            );
                            setSitePhone(siteDetail?.contactPhone || "");
                            setSiteEmergencyPhone(
                              siteDetail?.emergencyServicesPhone || "",
                            );
                          }}
                        >
                          <Text className="text-2xl">⚙️</Text>
                        ...
                        siteBuildings.length > 0 && (
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              confirmAction(
                                "Test Drill Site",
                                `Are you sure you want to trigger a mass TEST DRILL for ALL buildings in ${siteName}?`,
                                () =>
                                  triggerSiteIncident({
                                    clerkId: user?.id || "",
                                    siteName,
                      ...
                      <View className="bg-amber-900/40 p-4 border-b border-neutral-700/50 flex-row items-center">
                        ...
                        <View className="flex-1">
                          ...
                          <Text className="text-amber-400/80 text-xs">
                            Tap the ⚙️ icon to configure the fire service info
                            for this site.
                          </Text>
                        </View>
                      </View>
                        ...
                        {allInSiteActive ? (
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              confirmAction(
                                isRealEmergency
                                  ? "Resolve Site Emergency"
                                  : "Resolve Site Drill",
                                isRealEmergency
                                  ? `Are you sure you want to end the real evacuation for all buildings in ${siteName}?`
                                  : `Are you sure you want to end the test drill for all buildings in ${siteName}?`,
                                () =>
                                  resolveSiteIncident({
                                    clerkId: user?.id || "",
                                    siteName,
                          ...
                          >
                            <Text className="text-white text-base font-bold text-center">
                              ✅ Resolve{" "}
                              {isRealEmergency ? "Entire Site" : "Site Drill"}
                            </Text>
                          </TouchableOpacity>
                        ) : !isSiteComplete ? (
                          ...
                          <View className="flex-row space-x-2 mb-2">
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                confirmAction(
                                  isRealEmergency
                                    ? "Evacuate Rest of Site"
                                    : "Drill Rest of Site",
                                  isRealEmergency
                                    ? `Some buildings are already evacuating. Trigger real alarms for the remaining buildings in ${siteName}?`
                                    : `Some buildings are already drilling. Trigger test alarms for the remaining buildings in ${siteName}?`,
                                  () =>
                                    triggerSiteIncident({
                                      clerkId: user?.id || "",
                                      siteName,
                                      isDrill: !isRealEmergency,
                                    }),
                                  isRealEmergency ? "danger" : "warning",
                                )
                              }
                            >
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                confirmAction(
                                  isRealEmergency
                                    ? "Resolve Partial Evacuation"
                                    : "Resolve Partial Drill",
                                  isRealEmergency
                                    ? `Are you sure you want to resolve the ongoing real alarms for the evacuating buildings in ${siteName}?`
                                    : `Are you sure you want to resolve the ongoing test drills in ${siteName}?`,
                                  () =>
                                    resolveSiteIncident({
                                      clerkId: user?.id || "",
                                      siteName,
                            ...
                            >
                              <Text className="text-white text-xs font-bold text-center">
                                Resolve Partial{"\n"}
                                {isRealEmergency ? "Evacuation" : "Drill"}
                              </Text>
                            </TouchableOpacity>
                          </View>
                        ) : (
                          <View>
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                confirmAction(
                                  "Evacuate Site",
                                  `Are you sure you want to trigger a mass evacuation for ALL buildings in ${siteName}?`,
                                  () =>
                                    triggerSiteIncident({
                                      clerkId: user?.id || "",
                                      siteName,
                            ...
                            >
                              <Text
                                className="text-white text-base font-black tracking-widest text-center"
                                numberOfLines={1}
                                adjustsFontSizeToFit
                                minimumFontScale={0.5}
                              >
              ...
              {independent.length > 0 && (
                <View className="mb-10 bg-neutral-900 border border-neutral-800 rounded-[32px] p-5 shadow-lg ">
                  <Text className="text-xl font-bold text-white mb-5 border-b border-neutral-800 pb-3 px-1">
      ...
      {Boolean(isRegistering) && (
        ...
        >
          <ScrollView
            keyboardShouldPersistTaps="handled"
            className="flex-1 bg-neutral-900 pt-12 px-6"
            contentContainerStyle={{ paddingBottom: 120 }}
          >
            ...
            <View className="flex-row items-center mb-2">
              <Text className="text-neutral-300 font-bold">
                Draw Footprint ({bPins.length} pins)
              </Text>
              <TouchableOpacity
                className="bg-red-900/40 w-6 h-6 rounded-full items-center justify-center ml-2 border border-red-500/50"
                onPress={() =>
                  alert(
                    "Draw the pins sequentially around the outside wall of the building. Do not mix up the order or criss-cross the lines across the middle of the building. If the lines cross, the detection will fail.",
                  )
                }
              >
            ...
            <Text className="text-neutral-500 text-xs mb-4">
              Tap on the map to drop pins sequentially around the perimeter.
              Minimum 4 pins required.
            </Text>
            ...
            {hasSelfIntersection(bPins) && (
              <View className="bg-red-900/30 p-3 rounded-xl border border-red-500/50 mb-4 flex-row items-center">
                ...
                <Text className="text-red-400 text-xs flex-1 font-bold">
                  Polygon lines are criss-crossing! Please clear and draw the
                  perimeter sequentially.
                </Text>
              </View>
            )}

            <View
              className="bg-neutral-800 rounded-2xl  mb-6 border border-neutral-700 relative"
              style={{ height: 300 }}
            >
              {!showAddMap ? (
                <View className="flex-1 justify-center items-center p-6">
                  <ActivityIndicator color="white" />
                  <Text className="text-neutral-400 mt-4 text-xs font-bold">
                ...
                <View className="flex-1 justify-center items-center p-6">
                  <Text className="text-white text-center mb-4">
                    Interactive Maps are not supported in the Web Simulator.
                  </Text>
                  <Text className="text-neutral-400 text-center text-sm mb-4">
                    Please open this Admin Console on a physical mobile device
                    via Expo Go to use the Pin Drop feature.
                  </Text>
                  <View className="flex-row items-center bg-yellow-900/50 p-4 rounded-xl border border-yellow-700">
                    ...
                    <Text className="text-yellow-500 text-xs flex-1">
                      Web Fallback: We will generate 4 fake pins for testing.
                    </Text>
                  </View>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      showToast(
                        "4 Test Pins Generated! You can now save the building.",
                      );
                    }}
                  >
                    <Text className="text-white font-bold">
                      {bPins.length > 0
                        ? "Test Polygon Generated!"
                ...
                <>
                  ...
                  >
                    {bPins.map((pin, i) => (
                      <Marker
                        key={i}
                        coordinate={{
                          latitude: parseFloat(pin.lat as any) || 0,
                          longitude: parseFloat(pin.lon as any) || 0,
                        }}
                        ...
                        onDragEnd={(e: any) => {
                          ...
                          newPins[i] = {
                            ...newPins[i],
                            lat: parseFloat(
                              e.nativeEvent.coordinate.latitude.toFixed(6),
                            ),
                            lon: parseFloat(
                              e.nativeEvent.coordinate.longitude.toFixed(6),
                            ),
                          };
                          setBPins(newPins);
                        }}
                    ...
                    <Polygon
                      coordinates={
                        bPins.length > 2
                          ? bPins.map((p) => ({
                              latitude: parseFloat(p.lat as any) || 0,
                              longitude: parseFloat(p.lon as any) || 0,
                            }))
                          : [
                  ...
                  {!bName && (
                    <View className="absolute inset-0 bg-neutral-900/90 justify-center items-center p-6 z-10">
                      ...
                      <Text className="text-white font-bold text-center">
                        Enter a Building Name first
                      </Text>
                      <Text className="text-neutral-400 text-center text-xs mt-2">
                        The map drawing tools will unlock once you name the
                        building above.
                      </Text>
                    </View>
                  )}
              ...
              <View className="bg-neutral-800 rounded-2xl p-4 border border-neutral-700 mb-6">
                <View className="flex-row items-center mb-2">
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      alert(
                        "Ensure the coordinates trace the perimeter sequentially. Mixing the order will cause lines to criss-cross and break the detection math.",
                      )
                    }
                  >
                    <Text className="text-red-500 font-bold text-xs">?</Text>
                ...
                <Text className="text-neutral-400 text-sm mb-4">
                  You can manually fine-tune the exact GPS coordinates and
                  labels of your pins below. Useful for fixing inaccurate taps.
                </Text>
                ...
                {bPins &&
                  hasSelfIntersection(
                    bPins.map((p) => ({
                      ...p,
                      lat: parseFloat(p.lat as any) || 0,
                      lon: parseFloat(p.lon as any) || 0,
                    })),
                  ) && (
                    <View className="bg-red-900/30 p-3 rounded-xl border border-red-500/50 mb-4 flex-row items-center">
                      <Text className="text-red-500 mr-2">⚠️</Text>
                      <Text className="text-red-400 text-xs flex-1 font-bold">
                        Polygon lines are criss-crossing! Please adjust the
                        coordinates so they trace sequentially without crossing.
                      </Text>
                    </View>
                  )}

                <Text className="text-blue-400 text-xs mb-4 font-bold leading-5 bg-blue-900/20 p-2 rounded border border-blue-900/50">
                  ℹ️ Important: Points must be ordered sequentially (either
                  clockwise or counter-clockwise) tracing the outside perimeter
                  of the building.
                </Text>
                {bPins?.map((p: any, i: number) => (
                  <View
                    key={i}
                    className="bg-neutral-800 rounded-xl p-3 mb-3 border border-neutral-700 flex-row"
                  >
                    <View className="flex-1">
                      <Text className="text-neutral-500 text-[10px] uppercase font-bold mb-1 ml-1">
                        Label
                      </Text>
                      <TextInput
                ...
                <View className="flex-row justify-between mt-4">
                  ...
                  <TouchableOpacity
                    onPress={() => setBPins([])}
                    className="bg-red-900/40 px-3 py-2 rounded-lg border border-red-500/50"
                  >
            ...
            <Text className="text-neutral-300 font-bold mb-2">
              Master Plan Image
            </Text>
            <TouchableOpacity
            ...
            >
              {bImageUri ? (
                <View className="w-full relative">
                  <Image
                    source={{ uri: bImageUri }}
                    className="w-full h-40 rounded-lg mb-2 bg-neutral-900"
                    resizeMode="cover"
                  />
                  {Platform.OS === "web" && (
                    <View
                      className="absolute inset-0 items-center justify-center bg-black/60 rounded-lg"
                    ...
                    >
                      ...
                      <Text className="text-white/70 text-xs text-center px-4 mt-1">
                        (Web preview may appear blank, but the file is attached)
                      </Text>
                    </View>
                  )}
                </View>
      ...
      {showCalibrationWalk && selectedBuilding && (
        <AdminCalibrationWalk
          ...
          imageUrl={selectedBuilding.masterPlanUrl}
          calibPoints={selectedBuilding.imageCalibrationPoints || []}
          doorPins={selectedBuilding.doorPins || []}
        />
      )}
        ...
        >
          {selectedBuilding && (
            ...
            >
              ...
              {(() => {
                const isActive =
                  selectedBuilding?.polygon &&
                  selectedBuilding.polygon.length >= 3 &&
                  selectedBuilding.masterPlanId &&
                  selectedBuilding.imageCalibrationPoints &&
                  selectedBuilding.imageCalibrationPoints.length >= 3 &&
                  selectedBuilding.gridPaths &&
                  selectedBuilding.gridPaths.some((n: any) => n.isExit);
                if (!isActive) {
                  return (
                    <View className="bg-red-900/30 p-4 rounded-xl border border-red-500/50 mb-6 flex-row items-center">
                      ...
                      <View className="flex-1">
                        ...
                        <Text className="text-red-400 text-xs">
                          This building will not be pushed to guests until the
                          Polygon, Master Plan, Calibration, and Safe Routes are
                          fully set.
                        </Text>
                      </View>
                    </View>
                  ...
                  return (
                    <View className="bg-green-900/30 p-4 rounded-xl border border-green-700/50 mb-6 flex-row items-center">
                      ...
                      <View className="flex-1">
                        <Text className="text-green-500 font-bold mb-1">
                          Building is Active
                        </Text>
                        <Text className="text-green-400 text-xs">
                          This building is fully configured and is actively
                          being pushed to guests.
                        </Text>
                      </View>
                    </View>
                  );
              ...
              <View className="bg-neutral-800 p-6 rounded-2xl border border-neutral-700 mb-6">
                ...
                <TouchableOpacity
                  ...
                  disabled={
                    editBName === selectedBuilding.name &&
                    editBSite === (selectedBuilding.siteName || "") &&
                    (editBAddress === selectedBuilding.address ||
                      (editBAddress === "" &&
                        selectedBuilding.address === "No Address Provided"))
                  }
                  onPress={async () => {
                    try {
                      const finalAddress =
                        editBAddress || "No Address Provided";
              ...
              >
                {!showEditMap ? (
                  ...
                  <View className="flex-1 justify-center items-center p-6">
                    ...
                    <Text className="text-neutral-400 text-center text-sm mb-4">
                      Please open this Admin Console on a physical mobile device
                      via Expo Go to view or edit the Polygon pins.
                    </Text>
                    <View className="flex-row items-center bg-green-900/50 p-4 rounded-xl border border-green-700 mb-4">
                      ...
                      <Text className="text-green-500 text-xs flex-1">
                        This building has{" "}
                        {selectedBuilding.polygon?.length || 0} pins saved.
                      </Text>
                    </View>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        showToast(
                          "4 Test Pins Generated! You can now update the building footprint.",
                        );
                      }}
                    >
                      <Text className="text-white font-bold">
                  ...
                  <>
                    <MapView
                      ...
                      onMapReady={() => {
                        if (
                          location &&
                          (!selectedBuilding?.polygon ||
                            selectedBuilding.polygon.length === 0)
                        ) {
                          editMapRef.current?.animateToRegion(
                            {
                              latitude: location.coords.latitude,
                    ...
                    {editingPins &&
                      editingPins !== selectedBuilding.polygon &&
                      editingPins.length >= 3 && (
                        <TouchableOpacity
              ...
              {editingPins && editingPins.length > 0 && (
                <View className="bg-neutral-800 rounded-xl p-4 mb-6 border border-neutral-700">
                  <View className="flex-row justify-between items-center mb-2">
                    <Text className="text-white font-bold">
                      Selected Pins ({editingPins.length})
                    </Text>
                    <TouchableOpacity
                      onPress={() => setEditingPins(editingPins.slice(0, -1))}
                      className="bg-red-900/40 px-3 py-1 rounded-full border border-red-500/50"
                    >
                      <Text className="text-red-400 text-xs">
                        Undo Last Pin
                      </Text>
                  ...
                  {editingPins.map((pin: any, i: number) => (
                    <View
                      key={i}
                      className="flex-row justify-between items-center py-2 border-b border-neutral-700/50"
                    >
              ...
              <Text className="text-neutral-400 text-sm mb-4">
                You can manually fine-tune the exact GPS coordinates of your
                pins below. Useful for fixing inaccurate taps.
              </Text>
                  ...
                  >
                    ...
                    <View className="ml-3 pl-3 border-l border-neutral-700 justify-between items-center w-12 py-1">
                      ...
                      <TouchableOpacity
                        className="w-8 h-8 rounded-full items-center justify-center bg-red-900/40 border border-red-500/50"
                        onPress={() => {
                          const newPins = editingPins.filter(
                            (_: any, index: number) => index !== i,
                          );
                          setEditingPins(newPins);
                        }}
                  ...
                  {editingPins &&
                    ...
                    !hasSelfIntersection(
                      ...
                      <TouchableOpacity
                        ...
                        onPress={async () => {
                          try {
                            const parsedPins = editingPins.map((p: any) => ({
                              ...p,
                              lat: parseFloat(p.lat) || 0,
                              lon: parseFloat(p.lon) || 0,
                            }));
                            ...
                            showToast(
                              "Coordinates manually updated successfully!",
                            );
                          } catch (e) {
                            showToast("Error saving polygon", "error");
                          }
              ...
              {selectedBuilding.masterPlanUrl ? (
                <View className="bg-neutral-800 border border-neutral-700 p-4 rounded-2xl items-center mb-6">
                  ...
                  <Text className="text-neutral-400 text-xs text-center mb-4">
                    This map will be pushed to Guests entering the footprint.
                  </Text>
                  ...
                  <View className="w-full mt-4">
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setMapEditorStep(1);
                        setStep1PanMode(null);
                        setGridPaintMode(null);
                        setStep1Zoom(1);
                    ...
                    {/* Phase 27: Master Floor Plan V2 */}
                    <TouchableOpacity
                      className="bg-indigo-600/30 border border-indigo-500 px-6 py-4 rounded-xl flex-row items-center justify-center shadow-lg w-full mb-4 mt-4"
                      onPress={() => {
                        setCalibPoints(
                          selectedBuilding.imageCalibrationPoints || [],
                        );
                        setGridPaths(selectedBuilding.gridPaths || []);
                        setDoorPins(selectedBuilding.doorPins || []);
                        setMapEditorStep(1);
                        setStep1PanMode(null);
                        setStep1Zoom(1);
                        setStep1PanOffset({ x: 0, y: 0 });
                    ...
                    >
                      <Text className="text-white font-extrabold text-lg uppercase tracking-wider">
                        📡 Master Floor Plan V2
                      </Text>
                    </TouchableOpacity>
                    {selectedBuilding.imageCalibrationPoints?.length !==
                      selectedBuilding.polygon?.length && (
                      <Text className="text-orange-400 text-xs text-center mt-2">
                        ⚠️ Calibration required before routing.
                      </Text>
                    )}
                  </View>
                </View>
                ...
                <View className="bg-neutral-800 border border-neutral-700 p-6 rounded-2xl items-center mb-6">
                  <Text className="text-neutral-400 text-center mb-4">
                    No floor plan image uploaded yet. Guests will not see a map.
                  </Text>
                  <TouchableOpacity
        ...
        <Modal visible={true} animationType="fade" transparent>
          <View className="flex-1 bg-black/80 justify-center items-center px-6">
            <View className="bg-neutral-900 border border-neutral-700 p-6 rounded-3xl w-full max-w-sm">
              ...
              <Text className="text-neutral-400 mb-6">
                Are you sure you want to delete {selectedBuilding?.name}? This
                action cannot be undone and will permanently remove all
                associated maps and layouts.
              </Text>
          ...
          <View className="flex-1 bg-neutral-900 pt-6 px-4">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-xl font-bold text-white">
                {isLocatingUser ? "Locate Trapped User" : "Manage Map Layout"}
              </Text>
              <TouchableOpacity
                onPress={() => {
            ...
            {isLocatingUser ? (
              (() => {
                ...
                const liveUser = locatingRollCall.find(
                  (r: any) => r.userName === initialTarget?.label,
                );
                ...
                if (selectedBuilding?.polygon && target) {
                  ...
                  for (
                    ...
                    const xj = poly[j].lon,
                      yj = poly[j].lat;
                    const intersect =
                      yi > target.lat !== yj > target.lat &&
                      target.lon <
                        ((xj - xi) * (target.lat - yi)) / (yj - yi) + xi;
                ...
                return (
                  <View className="flex-1">
                    <View className="bg-red-600 py-3 px-4 rounded-xl mb-4 shadow-[0_0_10px_rgba(220,38,38,0.5)]">
                      <Text className="text-white font-black text-xl md:text-2xl text-center uppercase tracking-wider">
                    ...
                    {!isInside ? (
                      <View
                        className="bg-neutral-800 rounded-xl mb-6 w-full items-center justify-center border border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]"
                        style={{ height: "70%" }}
                      >
                        <Text className="text-red-500 font-black text-2xl mb-2 text-center">
                          OUT OF BOUNDS
                        </Text>
                        <Text className="text-white font-bold text-center text-lg px-8">
                          User is no longer inside the building!
                        </Text>
                        <Text className="text-red-300 text-center mt-4 px-8">
                          They may have exited safely without pressing the 'I am
                          Safe' button.
                        </Text>
                      </View>
                    ) : (
                              ...
                              <View className="flex-1 justify-center items-center relative">
                                ...
                                >
                                  ...
                                  >
                                    <Image
                                      source={{
                                        uri: selectedBuilding.masterPlanUrl,
                                      }}
                                    ...
                                    {(() => {
                                      ...
                                      const liveUser = locatingRollCall.find(
                                        (r: any) =>
                                          r.userName === initialTarget?.label,
                                      );
                                      const target = initialTarget
                                        ? {
                                            label: initialTarget.label,
                                      ...
                                      const imgPts = rawImgPts.map(
                                        (p: any) => ({
                                          x: isLegacyPixels
                                            ? p.x / Math.max(1, imgLayout.w)
                                            : p.x,
                                          y: isLegacyPixels
                                            ? p.y / Math.max(1, imgLayout.h)
                                            : p.y,
                                        }),
                                      );

                                      const minLat = Math.min(
                                        ...poly.map((p: any) => p.lat),
                                      );
                                      const maxLat = Math.max(
                                        ...poly.map((p: any) => p.lat),
                                      );
                                      const minLon = Math.min(
                                        ...poly.map((p: any) => p.lon),
                                      );
                                      ...
                                      if (imgPts && imgPts.length >= 4) {
                                        const minCX = Math.min(
                                          ...imgPts.map((c: any) => c.x),
                                        );
                                        const maxCX = Math.max(
                                          ...imgPts.map((c: any) => c.x),
                                        );
                                        const minCY = Math.min(
                                          ...imgPts.map((c: any) => c.y),
                                        );
                                        const maxCY = Math.max(
                                          ...imgPts.map((c: any) => c.y),
                                        );
                    ...
                    <TouchableOpacity
                      ...
                      onPress={async () => {
                        import("react-native").then((rn) => {
                          if (rn.Share)
                            rn.Share.share({
                              message: `EMERGENCY! User ${editingPins?.[0]?.label} is trapped at coordinates: ${editingPins?.[0]?.lat}, ${editingPins?.[0]?.lon} in ${selectedBuilding?.name}. Dispatch help immediately!`,
                            });
                        });
                      }}
                    >
                      <Text className="text-white font-bold text-xl tracking-widest">
                        🚨 SHARE TO FIRE BRIGADE
                      </Text>
                    </TouchableOpacity>
                  </View>
                );
              ...
              <>
                ...
                <View className="flex-row border-b border-neutral-700 mb-4 pb-2">
                  ...
                  >
                    <Text
                      className={`font-bold text-center w-full px-1 ${mapEditorStep === 1 ? "text-white" : "text-neutral-400"}`}
                      adjustsFontSizeToFit
                      numberOfLines={1}
                      minimumFontScale={0.5}
                    >
                  ...
                  >
                    <Text
                      className={`font-bold text-center w-full px-1 ${mapEditorStep === 2 ? "text-white" : "text-neutral-400"}`}
                      adjustsFontSizeToFit
                      numberOfLines={1}
                      minimumFontScale={0.5}
                    >
                ...
                <ScrollView
                  ...
                  keyboardShouldPersistTaps="handled"
                  className="flex-1"
                  style={{ display: mapEditorStep === 1 ? "flex" : "none" }}
                  showsVerticalScrollIndicator={false}
                  contentContainerStyle={{ paddingBottom: 120 }}
                  scrollEnabled={step1PanMode !== true}
                >
                  <Text className="text-neutral-400 text-lg mb-4">
                    Select a GPS pin from the list below, then tap its matching
                    ...
                    {selectedBuilding?.polygon?.map((p: any, i: number) => {
                      return (
                        ...
                        >
                          ...
                          {calibPoints[i] && (
                            <Text className="text-green-400 text-[10px] mt-1 font-bold">
                              ✓ Placed
                            </Text>
                          )}
                        </TouchableOpacity>
                  ...
                  <View className="flex-row space-x-2 mb-2">
                    <TouchableOpacity
                      className={`flex-1 py-3 rounded-xl items-center border-2 ${step1PanMode === false ? "bg-blue-600 border-blue-400" : "bg-neutral-800 border-neutral-700"}`}
                      onPress={() => {
                        const newMode = step1PanMode === false ? null : false;
                        setStep1PanMode(newMode);
                        if (newMode === null) setActiveCalibIdx(-1);
                      }}
                    >
                      <Text className="text-white font-bold text-lg">
                        📍 Place Pin
                    ...
                    <TouchableOpacity
                      className={`flex-1 py-3 rounded-xl items-center border-2 ${step1PanMode === true ? "bg-amber-600 border-amber-400" : "bg-neutral-800 border-neutral-700"}`}
                      onPress={() => {
                        const newMode = step1PanMode === true ? null : true;
                        setStep1PanMode(newMode);
                        if (newMode === true) {
                          setActiveCalibIdx(-1);
                        }
                        if (newMode === true) {
                          scrollViewRef.current?.scrollTo({ y: Math.max(0, step1MapScrollOffsetY - (height - getDynamicMapHeight()) / 2), animated: true });
                        }
                      }}
                    >
                      <Text className="text-white font-bold text-lg">
                  ...
                  <View
                    ...
                    onLayout={(e) => {
                      setStep1MapScrollOffsetY(e.nativeEvent.layout.y);
                      setImgLayout({
                        w: Math.max(1, e.nativeEvent.layout.width),
                        h: Math.max(1, e.nativeEvent.layout.height),
                      });
                      if (Platform.OS === "web" && step1MapRef.current) {
                        const rect =
                          step1MapRef.current.getBoundingClientRect();
                        setStep1MapPos({
                          x: rect.left + window.scrollX,
                          y: rect.top + window.scrollY,
                        });
                      } else if (step1MapRef.current) {
                        step1MapRef.current.measureInWindow(
                          (x: number, y: number) => {
                            setStep1MapPos({ x, y });
                          },
                        );
                  ...
                  >
                    {step1PanMode !== null && (
                      <View className="absolute top-4 right-4" style={{ zIndex: 100, elevation: 100 }}>
                        <TouchableOpacity
                          className="bg-red-600 px-4 py-3 rounded-full shadow-lg flex-row items-center border-2 border-red-400"
                          onPress={() => {
                            setStep1PanMode(null);
                            setActiveCalibIdx(-1);
                          }}
                        >
                          ...
                          <Text className="text-white font-bold ml-1">Cancel {step1PanMode ? "Pan" : "Pin"}</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                    {selectedBuilding?.masterPlanUrl && (
                          ...
                          if (Platform.OS === "web" && e.nativeEvent.deltaY) {
                            setStep1Zoom((z) =>
                              Math.max(
                                ...
                                Math.min(
                                  5,
                                  z - (e.nativeEvent.deltaY > 0 ? 0.2 : -0.2),
                                ),
                              ),
                            );
                          }
                      ...
                      >
                        ...
                        >
                          <View
                            ...
                            onStartShouldSetResponder={() => step1PanMode !== null}
                            onMoveShouldSetResponder={() => step1PanMode !== null}
                          ...
                          >
                            {calibPoints.map((p, i) => {
                              if (!p) return null;
                              const labelStr =
                                selectedBuilding?.polygon?.[i]?.label;
                              const labelShort = labelStr
                                ? labelStr
                                    .split(" ")
                                    .map((w: string) => w[0])
                                    .join("")
                              ...
                              const isLegacyPixels = p.x > 2;
                              const bounds = getRenderedImageBounds();
                              const px = isLegacyPixels
                                ? p.x
                                : bounds.offsetX + p.x * bounds.renderW;
                              const py = isLegacyPixels
                                ? p.y
                                : bounds.offsetY + p.y * bounds.renderH;
                              ...
                              return (
                                ...
                                >
                                  {activeCalibIdx === i ? (
                                    ...
                                    >
                                      <View
                                        className="absolute bg-red-500 shadow-md shadow-black"
                                        style={{
                                          left: 15,
                                          top: 0,
                                          width: 2,
                                      ...
                                      return (
                                        ...
                                        >
                                          <TouchableOpacity
                                            onPress={(e) => {
                                              e.stopPropagation();
                                              handleNudgeCalib(i, 0, -1);
                                            }}
                                            className="absolute top-1 p-2 bg-neutral-700 rounded-full"
                                          >
                                            <MaterialCommunityIcons
                                              name="chevron-up"
                                              size={24}
                                          ...
                                          <TouchableOpacity
                                            ...
                                            className="absolute bottom-1 p-2 bg-neutral-700 rounded-full"
                                          >
                                            <MaterialCommunityIcons
                                              name="chevron-down"
                                              size={24}
                                          ...
                                          <TouchableOpacity
                                            onPress={(e) => {
                                              e.stopPropagation();
                                              handleNudgeCalib(i, -1, 0);
                                            }}
                                            className="absolute left-1 p-2 bg-neutral-700 rounded-full"
                                          >
                                            <MaterialCommunityIcons
                                              name="chevron-left"
                                              size={24}
                                          ...
                                          <TouchableOpacity
                                            ...
                                            className="absolute right-1 p-2 bg-neutral-700 rounded-full"
                                          >
                                            <MaterialCommunityIcons
                                              name="chevron-right"
                                              size={24}
                  ...
                  <View className="flex-row justify-between mb-0">
                    <TouchableOpacity
                      className="bg-neutral-700 px-6 py-3 rounded-xl flex-1 mr-2 items-center justify-center"
                      onPress={() => {
                        setCalibPoints([]);
                        setActiveCalibIdx(0);
                      }}
                ...
                <View
                  className="flex-1"
                  style={{ display: mapEditorStep === 2 ? "flex" : "none" }}
                >
                    <ScrollView
                      ...
                      showsVerticalScrollIndicator={false}
                      contentContainerStyle={{ paddingBottom: 120 }}
                      scrollEnabled={mapEditorMode === "v1" ? gridPaintMode !== "pan" : step2PinType !== "pan"}
                    >
                      {mapEditorMode === "v1" && (
                        <>
                          <Text className="text-neutral-400 text-lg mb-4">
                            Select a brush type, then tap the map to paint grid
                            cells.
                          </Text>
                          ...
                          <View className="flex-row space-x-2 mb-2">
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                const newMode = gridPaintMode === "pan" ? null : "pan";
                                setGridPaintMode(newMode as any);
                                if (newMode === "pan") {
                                  step2ScrollViewRef.current?.scrollTo({ y: Math.max(0, step2MapScrollOffsetY - (height - getDynamicMapHeight()) / 2), animated: true });
                                }
                              }}
                            >
                      ...
                      <View className="flex-row justify-between items-center mb-4">
                        {mapEditorMode === "v1" && (
                          <View className="flex-row space-x-2">
                            <Text className="text-white my-auto font-bold mr-2">
                              Grid Size:
                            </Text>
                            <TouchableOpacity
                              className="bg-neutral-700 p-2 rounded-lg"
                              onPress={() =>
                                setGridSizeMeters((s) => Math.max(0.5, s - 0.1))
                              }
                            >
                              <MaterialCommunityIcons
                                name="minus"
                            ...
                            <Text className="text-white my-auto font-bold w-12 text-center">
                              {gridSizeMeters.toFixed(1)}m
                            </Text>
                            <TouchableOpacity
                              className="bg-neutral-700 p-2 rounded-lg"
                              onPress={() =>
                                setGridSizeMeters((s) => Math.min(5.0, s + 0.1))
                              }
                            >
                              <MaterialCommunityIcons
                                name="plus"
                      ...
                      <View
                        ...
                        style={{ height: getDynamicMapHeight() }}
                        onLayout={(e) => {
                          setStep2MapScrollOffsetY(e.nativeEvent.layout.y);
                          setImgLayout({
                            w: Math.max(1, e.nativeEvent.layout.width),
                            h: Math.max(1, e.nativeEvent.layout.height),
                          });
                          if (Platform.OS === "web" && step2MapRef.current) {
                            const rect =
                              step2MapRef.current.getBoundingClientRect();
                            setStep2MapPos({
                              x: rect.left + window.scrollX,
                              y: rect.top + window.scrollY,
                            });
                          } else if (step2MapRef.current) {
                            step2MapRef.current.measureInWindow(
                              (x: number, y: number) => {
                                setStep2MapPos({ x, y });
                              },
                            );
                      ...
                      >
                        {mapEditorMode === "v1" && gridPaintMode !== null && (
                          <View className="absolute top-4 right-4" style={{ zIndex: 100, elevation: 100 }}>
                            <TouchableOpacity
                              className="bg-red-600 px-4 py-3 rounded-full shadow-lg flex-row items-center border-2 border-red-400"
                              onPress={() => setGridPaintMode(null as any)}
                            >
                              <MaterialCommunityIcons name="close" size={20} color="white" />
                              <Text className="text-white font-bold ml-1">Cancel Tool</Text>
                            </TouchableOpacity>
                          </View>
                        )}
                        {mapEditorMode === "v2" && step2PinType !== null && (
                          <View className="absolute top-4 right-4" style={{ zIndex: 100, elevation: 100 }}>
                            <TouchableOpacity
                              className="bg-red-600 px-4 py-3 rounded-full shadow-lg flex-row items-center border-2 border-red-400"
                              onPress={() => {
                                setStep2PinType(null);
                                setActiveDoorPinIdx(-1);
                              }}
                            >
                              <MaterialCommunityIcons name="close" size={20} color="white" />
                              <Text className="text-white font-bold ml-1">Cancel {step2PinType === "pan" ? "Pan" : step2PinType === "entrance" ? "Entrance Pin" : "Fire Exit Pin"}</Text>
                            </TouchableOpacity>
                          </View>
                        )}
                        {selectedBuilding?.masterPlanUrl && (
                                ...
                                setStep1Zoom((z) =>
                                  Math.max(
                                    ...
                                    Math.min(
                                      5,
                                      z -
                                        (e.nativeEvent.deltaY > 0 ? 0.2 : -0.2),
                                    ),
                                  ),
                                );
                              }
                          ...
                          >
                            ...
                            >
                              <View
                                ...
                                onStartShouldSetResponder={() => mapEditorMode === "v1" ? gridPaintMode !== null : step2PinType !== null}
                                onMoveShouldSetResponder={() => mapEditorMode === "v1" ? gridPaintMode !== null : step2PinType !== null}
                                onResponderTerminationRequest={() => false}
                                onResponderGrant={(e) => {
                                  setIsMapInteracting(true);
                                  handleTouchStart(
                                    ...
                                    step1Zoom,
                                    step1PanOffset,
                                    mapEditorMode === "v1" ? gridPaintMode === "pan" : step2PinType === "pan",
                                    mapEditorMode === "v1" ? handleGridInteraction : handleStep2DoorPinPaint,
                                  );
                                }}
                                onResponderMove={(e) =>
                                  handleTouchMove(
                              ...
                              >
                                ...
                                {mapEditorMode === "v1" && (() => {
                                  ...
                                  const gridLines = [];
                                  const reqPins =
                                    selectedBuilding?.polygon?.length || 4;
                                  const currentCalib =
                                    calibPoints.filter(Boolean).length >=
                                    reqPins
                                      ? calibPoints.filter(Boolean)
                                      : selectedBuilding?.imageCalibrationPoints;
                                  if (
                                    ...
                                    if (tl && br) {
                                      ...
                                      const brY = isLegacy
                                        ...
                                        : bounds.offsetY +
                                          br.y * bounds.renderH;
                                      const tlGrid = mapGPSToImage(
                                        maxLat,
                                        minLon,
                                      );
                                      const brGrid = mapGPSToImage(
                                        maxLat - rows * cellLatSpan,
                                        minLon + cols * cellLonSpan,
                                      );
                                      if (tlGrid && brGrid) {
                                        const tlxG = isLegacy
                                          ? tlGrid.x
                                          : bounds.offsetX +
                                            tlGrid.x * bounds.renderW;
                                        const brxG = isLegacy
                                          ? brGrid.x
                                          : bounds.offsetX +
                                            brGrid.x * bounds.renderW;
                                        const tlyG = isLegacy
                                          ? tlGrid.y
                                          : bounds.offsetY +
                                            tlGrid.y * bounds.renderH;
                                        const bryG = isLegacy
                                          ? brGrid.y
                                          : bounds.offsetY +
                                            brGrid.y * bounds.renderH;

                                        for (let c = 0; c <= cols; c++) {
                                          const lx =
                                            tlxG + (c / cols) * (brxG - tlxG);
                                          gridLines.push(
                                            <View
                                              key={`v-${c}`}
                                              style={{
                                  ...
                                  return (
                                    <>
                                      ...
                                      {gridPaths.map((p) => {
                                        const latCenter =
                                          maxLat - (p.row + 0.5) * cellLatSpan;
                                        const lonCenter =
                                          minLon + (p.col + 0.5) * cellLonSpan;
                                        const pu = mapGPSToImage(
                                          latCenter,
                                          lonCenter,
                                        );
                                        if (!pu) return null;
                                        ...
                                        const cellTopLeft = mapGPSToImage(
                                          maxLat - p.row * cellLatSpan,
                                          minLon + p.col * cellLonSpan,
                                        );
                                        const cellBottomRight = mapGPSToImage(
                                          maxLat - (p.row + 1) * cellLatSpan,
                                          minLon + (p.col + 1) * cellLonSpan,
                                        );
                                        ...
                                        return (
                                          <View
                                            ...
                                            className={`absolute items-center justify-center border ${p.type === "entrance" ? "bg-blue-600/80 border-blue-500" : p.isExit ? "bg-green-600/80 border-green-500" : "bg-lime-500/80 border-lime-400"}`}
                                            style={{
                                              pointerEvents: "none",
                                              left: finalLeft,
                                              top: finalTop,
                                              width: cellW,
                                              height: cellH,
                                              zIndex: p.isExit || p.type === "entrance" ? 10 : 1,
                                            }}
                                          >
                                            {(p.isExit || p.type === "entrance") && (
                                              <Text
                                                style={{
                                                  pointerEvents: "none",
                                                }}
                              ...
                              {mapEditorMode === "v2" && (
                                ...
                                >
                                  <>
                                    {/* V2: Render Static Building Pins (Green) */}
                                    {calibPoints.map((p, i) => {
                                      if (!p) return null;
                                      const bounds = getRenderedImageBounds();
                                    ...
                                    {/* V2: Render Door Pins (Entrance = Blue, Exit = Red) */}
                                    {doorPins.map((p, i) => {
                                      const bounds = getRenderedImageBounds();
                                      const px = p.x > 2 ? p.x : bounds.offsetX + p.x * bounds.renderW;
                                      const py = p.y > 2 ? p.y : bounds.offsetY + p.y * bounds.renderH;
                                      ...
                                      return (
                                        <TouchableOpacity
                                          ...
                                          style={{
                                            ...
                                            width: 40,
                                            height: 40,
                                            zIndex: activeDoorPinIdx === i ? 10 : 1,
                                          }}
                                          onPress={(e) => {
                                            e.stopPropagation();
                                            if (activeDoorPinIdx === i) {
                                              setActiveDoorPinIdx(-1);
                                            } else {
                                              setActiveDoorPinIdx(i);
                                              setStep1PanMode(false);
                                            }
                                          }}
                                        >
                                          {activeDoorPinIdx === i ? (
                                            <View className="absolute pointer-events-none" style={{ left: 4, top: 4, width: 32, height: 32 }}>
                                              <View className={`absolute shadow-md shadow-black ${p.type === "entrance" ? "bg-blue-500" : "bg-red-500"}`} style={{ left: 15, top: 0, width: 2, height: 12 }} />
                                              <View className={`absolute shadow-md shadow-black ${p.type === "entrance" ? "bg-blue-500" : "bg-red-500"}`} style={{ left: 15, bottom: 0, width: 2, height: 12 }} />
                                              <View className={`absolute shadow-md shadow-black ${p.type === "entrance" ? "bg-blue-500" : "bg-red-500"}`} style={{ top: 15, left: 0, width: 12, height: 2 }} />
                                              <View className={`absolute shadow-md shadow-black ${p.type === "entrance" ? "bg-blue-500" : "bg-red-500"}`} style={{ top: 15, right: 0, width: 12, height: 2 }} />
                                            </View>
                                          ) : (
                                            <>
                                              <MaterialCommunityIcons
                                          ...
                                          {activeDoorPinIdx === i && (() => {
                                            ...
                                            return (
                                              ...
                                              >
                                                <TouchableOpacity onPress={(e) => { e.stopPropagation(); handleNudgeDoorPin(i, 0, -1); }} className="absolute top-1 p-2 bg-neutral-700 rounded-full">
                                                  <MaterialCommunityIcons name="chevron-up" size={24} color="white" />
                                                </TouchableOpacity>
                                                <TouchableOpacity onPress={(e) => { e.stopPropagation(); handleNudgeDoorPin(i, 0, 1); }} className="absolute bottom-1 p-2 bg-neutral-700 rounded-full">
                                                  <MaterialCommunityIcons name="chevron-down" size={24} color="white" />
                                                </TouchableOpacity>
                                                <TouchableOpacity onPress={(e) => { e.stopPropagation(); handleNudgeDoorPin(i, -1, 0); }} className="absolute left-1 p-2 bg-neutral-700 rounded-full">
                                                  <MaterialCommunityIcons name="chevron-left" size={24} color="white" />
                                                </TouchableOpacity>
                                                <TouchableOpacity onPress={(e) => { e.stopPropagation(); handleNudgeDoorPin(i, 1, 0); }} className="absolute right-1 p-2 bg-neutral-700 rounded-full">
                      ...
                      {mapEditorMode === "v1" && (
                        <View className="bg-blue-900/30 p-4 rounded-xl border border-blue-600/50 mb-6 flex-row">
                          ...
                          <View className="flex-1">
                            <Text className="text-blue-400 font-bold mb-1 uppercase tracking-wider text-lg">
                              Grid Strategy
                            </Text>
                            <Text className="text-blue-300 text-base">
                              ...
                              along corridors and rooms to define where users can
                              walk. Mark{" "}
                              <Text className="font-bold text-white text-base">
                                Exits
                              </Text>{" "}
                              at the doors. The system automatically calculates
                              the shortest path through painted routes.
                            </Text>
                          </View>
                        </View>
                      )}
                      ...
                      {mapEditorMode === "v1" ? (
                        <View className="flex-row justify-between mb-0">
                          <TouchableOpacity
                            className="flex-1 bg-neutral-700 py-3 rounded-xl items-center justify-center mr-2"
                            onPress={() => setGridPaths([])}
                          >
                            <Text className="text-white font-bold text-lg text-center">
                              Clear Grid
                            </Text>
                          ...
                          {(() => {
                            ...
                            return (
                              <TouchableOpacity
                                ...
                                onPress={async () => {
                                  try {
                                    await updateBuildingCalibration({
                                      clerkId: user?.id || "",
                                      buildingId: selectedBuilding._id,
                                      calibrationPoints: calibPoints,
                                    });
                                    await updateBuildingGridPaths({
                                      clerkId: user?.id || "",
                                      buildingId: selectedBuilding._id,
                                      gridPaths,
                                    });
                                    setSelectedBuilding({
                                      ...selectedBuilding,
                                      imageCalibrationPoints: calibPoints,
                                      gridPaths: gridPaths,
                                    });
                                    setIsMapEditorOpen(false);
                                    showToast(
                                      "Floor plan configured successfully!",
                                    );
                                  } catch (e) {
                        ...
                        <View className="flex-col mb-0">
                          <View className="flex-col mb-4 bg-neutral-800 p-4 rounded-xl border border-neutral-700">
                            <Text className="text-white font-bold mb-3 uppercase tracking-wider text-sm">Active Pin Type</Text>
                            <View className="flex-row justify-between mb-4">
                              <TouchableOpacity
                                className={`flex-1 py-3 px-2 rounded-xl flex-row items-center justify-center mr-2 ${step2PinType === "entrance" ? "bg-blue-600 border-2 border-blue-400" : "bg-neutral-700 border-2 border-transparent"}`}
                                onPress={() => {
                                  const newMode = step2PinType === "entrance" ? null : "entrance";
                                  setStep2PinType(newMode);
                                  if (newMode === null) setActiveDoorPinIdx(-1);
                                }}
                              >
                                <MaterialCommunityIcons name="login-variant" size={20} color="white" />
                                <Text className="text-white font-bold ml-1">Entrance</Text>
                              </TouchableOpacity>
                              <TouchableOpacity
                                className={`flex-1 py-3 px-2 rounded-xl flex-row items-center justify-center mr-2 ${step2PinType === "exit" ? "bg-red-600 border-2 border-red-400" : "bg-neutral-700 border-2 border-transparent"}`}
                                onPress={() => {
                                  const newMode = step2PinType === "exit" ? null : "exit";
                                  setStep2PinType(newMode);
                                  if (newMode === null) setActiveDoorPinIdx(-1);
                                }}
                              >
                                <MaterialCommunityIcons name="door-open" size={20} color="white" />
                                <Text className="text-white font-bold ml-1">Fire Exit</Text>
                              </TouchableOpacity>
                              <TouchableOpacity
                                className={`flex-1 py-3 px-2 rounded-xl flex-row items-center justify-center ${step2PinType === "pan" ? "bg-amber-600 border-2 border-amber-400" : "bg-neutral-700 border-2 border-transparent"}`}
                                onPress={() => {
                                  const newMode = step2PinType === "pan" ? null : "pan";
                                  setStep2PinType(newMode);
                                  if (newMode === "pan") {
                                    setActiveDoorPinIdx(-1);
                                    step2ScrollViewRef.current?.scrollTo({ y: Math.max(0, step2MapScrollOffsetY - (height - getDynamicMapHeight()) / 2), animated: true });
                            ...
                            <TouchableOpacity
                              className="bg-neutral-700 py-3 rounded-xl items-center justify-center mb-4"
                              onPress={() => setDoorPins([])}
                            >
                              <Text className="text-white font-bold">Clear All Pins</Text>
                            </TouchableOpacity>
                            <Text className="text-neutral-400 text-xs text-center mb-1 leading-5">Tap the map to place the selected pin type. Use the navigation tool to adjust, and the green checkmark to confirm.</Text>
                          </View>
                          
                          <View className="bg-indigo-900/30 border border-indigo-500 p-4 rounded-2xl mb-4 items-center">
                            <Text className="text-white font-bold mb-2">Safe Route Mapping (Wi-Fi)</Text>
                            <Text className="text-indigo-200 text-xs text-center mb-3">Walk the safe routes to map Wi-Fi signals for accuracy.</Text>
                            <TouchableOpacity 
                          ...
                          <TouchableOpacity
                            className="bg-green-600 py-4 rounded-xl items-center justify-center"
                            onPress={async () => {
                              try {
                                await updateBuildingCalibration({
                                  clerkId: user?.id || "",
                                ...
                                setIsMapEditorOpen(false);
                                showToast("Building setup saved successfully!");
                              } catch (e) {
          ...
          {manageSiteName && (
            ...
            >
              <View className="flex-row justify-between items-center mb-6">
                ...
                <TouchableOpacity
                  onPress={() => setManageSiteName(null)}
                  className="bg-neutral-800 p-2 rounded-full border border-neutral-700"
                >
                  <Text className="text-white font-bold">✕</Text>
                </TouchableOpacity>
              ...
              <View className="bg-neutral-800 p-6 rounded-2xl border border-neutral-700 mb-6">
                ...
                />
                <Text className="text-white font-bold mb-2">
                  Fire Service Description (Hazards & Assembly)
                </Text>
                <TextInput
                  className="bg-neutral-900 border border-neutral-700 rounded-xl p-4 text-white mb-4"
                  value={siteDesc}
                ...
                />
                <Text className="text-white font-bold mb-2">
                  Site Admin Name (Responsible Person)
                </Text>
                <TextInput
                  className="bg-neutral-900 border border-neutral-700 rounded-xl p-4 text-white mb-4"
                  value={siteAdminName}
                ...
                />
                <Text className="text-white font-bold mb-2">
                  Local Emergency Services / Fire Brigade Phone
                </Text>
                <TextInput
          ...
          <View className="flex-1 bg-black p-6 pt-10">
            <View className="flex-row justify-between items-center mb-6 border-b border-neutral-800 pb-4">
              <View>
                ...
                <Text className="text-neutral-400 mt-1">
                  Review past incidents and drills
                </Text>
              </View>
              <TouchableOpacity
            ...
            >
              <View className="mb-6 flex-row justify-end">
                <TouchableOpacity
                  onPress={exportLogs}
                  className="bg-neutral-800 px-4 py-3 rounded-xl border border-neutral-700 flex-row items-center"
                >
                  <MaterialCommunityIcons
                    name="download"
                    size={18}
              ...
              {!allIncidentsHistory || allIncidentsHistory.length === 0 ? (
                <View className="items-center py-10 bg-neutral-900 rounded-3xl border border-neutral-800">
                  ...
                  <Text className="text-neutral-500 text-sm mt-1">
                    Drills and evacuations will appear here
                  </Text>
                </View>
              ) : (
                Object.keys(groupedIncidents).map((monthKey) => (
                  <View key={monthKey} className="mb-8">
                            ...
                            async () => {
                              try {
                                await deleteIncidents({
                                  clerkId: user?.id || "",
                                  incidentIds: groupedIncidents[monthKey].map(
                                    (i: any) => i._id,
                                  ),
                                });
                    ...
                    {groupedIncidents[monthKey].map((inc: any) => {
                      const durationMs =
                        (inc.resolvedAt || Date.now()) - inc.triggeredAt;
                      ...
                      return (
                        <View
                          key={inc._id}
                          className="bg-neutral-900 border border-neutral-800 p-5 rounded-2xl mb-4 flex-row justify-between items-center shadow-lg"
                        >
                          <View>
                            <View className="flex-row items-center mb-1">
                              <Text
                                className={`font-black text-lg ${inc.isDrill ? "text-amber-500" : "text-red-500"}`}
                              >
                                {inc.isDrill
                                  ? "🔔 TEST DRILL"
                                  : "🚨 REAL EVACUATION"}
                          ...
                          <View className="bg-black/50 px-4 py-3 rounded-xl items-center border border-neutral-800">
                            <Text className="text-neutral-500 text-[10px] uppercase font-black tracking-widest mb-1">
                              Duration
                            </Text>
      ...
      {Boolean(confirmDialog.visible) && (
        <Modal visible={true} animationType="fade" transparent>
          <View className="flex-1 bg-black/80 justify-center items-center px-6">
            <View className="bg-neutral-900 border border-neutral-700 p-6 rounded-3xl w-full max-w-sm">
              <Text
                className={`text-xl font-black mb-2 tracking-wide ${confirmDialog.intent === "danger" ? "text-red-500" : confirmDialog.intent === "warning" ? "text-amber-500" : "text-green-400"}`}
              >
                {confirmDialog.title}
              </Text>
              <Text className="text-neutral-300 text-base mb-8 leading-relaxed">
